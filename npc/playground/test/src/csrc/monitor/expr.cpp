...
static uint64_t expr_eval(int p, int q);
...
uint64_t expr(char *e, bool *success) 
{
  ...
  uint64_t expr_result = expr_eval(0, nr_token);
...
static uint64_t expr_eval(int p, int q) 
{
  ...
  {
    /* Bad expression */
    printf("p>q!!!expr expr_eval error!\n");
    assert(0);
  }
  else if (p == q) 
  {
    ...
    else if(tokens[p].type == TK_REG)
    {
      ...
      uint64_t reg_value = find_reg_str2val(tokens[p].str, reg_flag);
      assert(*reg_flag);
      free(reg_flag);
      // printf("reg read not imple");
      // assert(0);
      return reg_value;
    }
  }
  ...
  else 
  {
    ...
    uint64_t val1 = expr_eval(p, op - 1);
    uint64_t val2 = expr_eval(op + 1, q);
